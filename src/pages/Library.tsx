 ...
 * Library.tsx
 * ─────────────────────────────────────────────────────────────
 * Apple Music–style Library page with:
 *  ✓ Vertical list navigation for Playlists, Artists, Albums, Songs
 *  ✓ Recently Added section below
  ...
  return (
    <div className="pb-4">
      ...
      {activeTab !== null && (
        <>
          {/* Back header */}
          <div className="px-2 pt-14 pb-2">
            <button
              onClick={() => {
                setActiveTab(null);
                setGenreFilter(null);
              }}
              className="flex items-center gap-0.5 text-[#fc3c44] active:opacity-50 transition-opacity"
            >
              <ChevronLeft size={24} strokeWidth={2.5} />
              <span className="text-[17px] font-medium">Library</span>
            </button>
          </div>
          ...
          {activeTab === 'songs' && (
            <div>
              <div className="px-5 pb-2">
                <h1 className="text-[34px] font-bold text-white">Songs</h1>
              </div>
              {tracksLoading && (
          ...
          {activeTab === 'albums' && (
            <div className="px-5">
              <h1 className="text-[34px] font-bold text-white mb-4">Albums</h1>
              {albumsLoading && (
                <div className="flex items-center justify-center py-20">
                  <Loader2 className="w-8 h-8 animate-spin text-[#fc3c44]" />
                </div>
          ...
          {activeTab === 'artists' && (
            <div>
              <div className="px-5 pb-2">
                <h1 className="text-[34px] font-bold text-white">Artists</h1>
              </div>
              {artistsLoading && (
                <div className="flex items-center justify-center py-20">
                  <Loader2 className="w-8 h-8 animate-spin text-[#fc3c44]" />
          ...
          {activeTab === 'playlists' && (
            <div className="px-5">
              <h1 className="text-[34px] font-bold text-white mb-4">Playlists</h1>
      ...
      {/* ── Main Library view (category list + Recently Added) ── */}
      {activeTab === null && (
        <>
          <div className="px-5 pt-14 pb-2 flex items-baseline justify-between">
            <h1 className="text-[34px] font-bold text-white">Library</h1>
          </div>
          ...
          <div className="px-5">
            {tabs.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className="w-full flex items-center gap-4 py-[14px] border-b border-white/[0.06] last:border-0 active:bg-white/[0.04] transition-colors text-left"
              >
                <Icon size={24} className="text-[#fc3c44] flex-shrink-0" />
                <span className="text-[20px] font-semibold text-white flex-1">{label}</span>
          ...
          <section className="mt-6">
            ...
            {albumsLoading ? (
              ...
              <div className="px-5 grid grid-cols-2 gap-x-4 gap-y-5">
                {uniqueAlbums.slice(0, 8).map((album) => (
                  <button
                    key={album.id}
                    onClick={() => onNavigate('album', album.id)}
                    className="text-left group active:scale-[0.97] transition-transform duration-150"
                  >
                    ...
                    <p className="text-[13px] font-semibold text-white truncate">{album.title}</p>
                    <p className="text-[12px] text-white/45 truncate">{album.artist}</p>
                  </button>
                ))}
              </div>
            )}
